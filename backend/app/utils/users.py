...
async def authenticate_user(
    *, session: AsyncSession, email_or_username: str, password: str
) -> User | None:
    ...
    if "@" in email_or_username:
        user = await get_user_by_email(session=session, email=email_or_username)
    else:
        user = await get_user_by_username(session=session, username=email_or_username)
